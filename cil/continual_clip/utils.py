...
def collate_fn_image_path(batch, transforms):
    """
    Collate function for datasets that return image paths.
    'batch' is a list of tuples, where each tuple is (image_path, label, ...).
    This version is more robust to malformed items in the batch.
    """
    # Filter out potential Nones or malformed entries
    ...
    labels = []
    for item in batch:
        # Each item should be a tuple/list with at least (path, label)
        if isinstance(item, (tuple, list)) and len(item) >= 2:
            paths.append(item[0])
            labels.append(item[1])
        else:
            # This helps in debugging what's coming from the dataloader
            print(f"WARNING: Skipping malformed item in batch. Item: {item}")

    if not paths:
        # If all items in the batch were malformed, return empty tensors
        return torch.tensor([]), torch.tensor([])

    try:
        images = [transforms(Image.open(p).convert("RGB")) for p in paths]
    except Exception as e:
        print(f"ERROR: Failed to load images from paths. First path: {paths[0] if paths else 'N/A'}. Error: {e}")
        # Return empty tensors to avoid crashing the main process
        return torch.tensor([]), torch.tensor([])
...
def get_class_ids_per_task(cfg: DictConfig) -> list:
    yield cfg.class_order[:cfg.initial_increment]
    for i in range(cfg.initial_increment, len(cfg.class_order), cfg.increment):
        yield cfg.class_order[i:i + cfg.increment]
