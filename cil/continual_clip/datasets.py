...
from continuum.datasets import (
    CIFAR10,
    CIFAR100, ImageNet100, TinyImageNet200, ImageFolderDataset, Core50, DomainNet
)
...
def get_dataset(cfg, is_train, transforms=None):
    if cfg.dataset == "cifar100":
        ...
        # The continuum DomainNet implementation doesn't expose class names directly.
        # We can get them by creating a torchvision ImageFolder dataset on one of the domains,
        # as all domains share the same classes.
        any_domain_path = os.path.join(data_path, "clipart")
        if not os.path.isdir(any_domain_path):
            raise FileNotFoundError(
                f"The 'clipart' domain was not found in {data_path}. "
                "The dataset appears to be in zip format. "
                "Please set `download=True` for the DomainNet dataset in your script to allow automatic extraction."
            )
        classes_names = torchdata.ImageFolder(any_domain_path).classes
