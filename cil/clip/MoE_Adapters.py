...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, text_or_image=None,cfg=None):
        ...
        self.top_k = 2
        self.ffn_num = 64
        self.experts_num = cfg.num_experts if cfg is not None and hasattr(cfg, 'num_experts') else 2
        self.softmax = nn.Softmax(1)
        self.softplus = nn.Softplus()
        self.noisy_gating = True
        self.adaptmlp_list = nn.ModuleList()
        self.shared_adaptmlp_list = nn.ModuleList()
        ...
        for i in range(self.experts_num):  #
            self.adaptmlp = Adapter(d_model=d_model, dropout=0.1, bottleneck=self.ffn_num,
                                    init_option='lora',
                                    adapter_scalar=cfg.adapter_scalar if cfg.adapter_scalar is not None else 0.1,
        ...
        for i in range(self.shared_expert_num):
            self.shared_adaptmlp = Adapter(d_model=d_model, dropout=0.1, bottleneck=self.ffn_num*self.experts_num,
                                    init_option='lora',
                                    adapter_scalar=cfg.adapter_scalar if cfg is not None and hasattr(cfg, 'adapter_scalar') else 0.1,
                                    adapter_layernorm_option='none',
                                    )
            self.shared_adaptmlp_list.append(self.shared_adaptmlp)
        ...
        if global_taskid is not None:
            ...
            if self.shared_expert_num > 0:
                for i in range(self.shared_expert_num):
                    ...
                    shared_output = self.shared_adaptmlp_list[i](x_reshaped, add_residual=False)
                    # 将输出reshape回原始形状
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, text_or_image=None,cfg=None):
        ...
        self.width = width
        self.layers = layers
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask, text_or_image,cfg=cfg) for _ in range(layers)])
...
class CLIP(nn.Module):
    ...
    def encode_text(self, text):
        # 添加调试信息
        print("\n=== Text Encoding Debug Info ===")
        print(f"Input text tensor shape: {text.shape}")
        
        # 检查特殊标记的位置
        sot_positions = (text == self.token_embedding.num_embeddings - 2).nonzero()  # <start_of_text>
        eot_positions = (text == self.token_embedding.num_embeddings - 1).nonzero()  # <end_of_text>
        
        print(f"Batch size: {text.shape[0]}")
        for b in range(text.shape[0]):
            print(f"\nBatch {b}:")
            print(f"SOT token position: {sot_positions[sot_positions[:, 0] == b, 1].item() if len(sot_positions) > 0 else 'Not found'}")
            print(f"EOT token position: {eot_positions[eot_positions[:, 0] == b, 1].item() if len(eot_positions) > 0 else 'Not found'}")
            
            # 尝试找到 {class} token
            tokens = text[b].tolist()
            for pos, token in enumerate(tokens):
                if self.token_embedding.num_embeddings > token >= 0:  # 确保token ID有效
                    decoded = self.tokenizer.decode([token]).strip()
                    if decoded == "{class}":
                        print(f"{{class}} token found at position {pos}")
        print("=============================\n")
...
def compress_clip_model(model, drop_visual_layers, drop_transformer_layers):
    ...
    if isinstance(model.visual, VisualTransformer):
        ...
        # 确保丢弃后的教师层数量正确
        # assert len(student_transformer_layers) == 4, "丢弃后 transformer 教师层应保留 4 层。"
        # 重新构建 transformer 的 resblocks
        #add_prologue(student_transformer_layers[0], None)
        #add_epilogue(student_transformer_layers[-1], None)
        # 修改后
        student_transformer_layers[0] = add_prologue(student_transformer_layers[0], None)
        student_transformer_layers[-1] = add_epilogue(student_transformer_layers[-1], None)
...
def build_MoE_Adapters_model(state_dict: dict,compress=True,cfg=None):
